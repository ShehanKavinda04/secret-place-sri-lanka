...
match = re.search(
    r"'lankarama'.*?'history_narrative' => <<<'EOD'\n(.*?)\nEOD",
    route_source,
    re.DOTALL,
)
if not match:
    raise RuntimeError('Lankarama narrative not found')

text = re.sub(r'\$([^$]+)\$', r'\1', match.group(1)).replace('\\', '')
output_path = os.path.join(os.path.dirname(__file__), 'public', 'audio', 'lankarama_history.mp3')
