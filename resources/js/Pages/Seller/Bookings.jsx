        ...
        >
            ...
            <div className="ml-3 w-0 flex-1">
                ...
                <p className="mt-1 text-sm opacity-90">
                    {notification.guest_name} booked{" "}
                    {notification.property_name}
                </p>
                <p className="mt-1 text-xs opacity-75">
                    {new Date(notification.check_in_date).toLocaleDateString()}{" "}
                    -{" "}
                    {new Date(notification.check_out_date).toLocaleDateString()}
                </p>
            </div>
            <button
                onClick={onClose}
...
export default function Bookings() {
    ...
    const stats = {
        checkInsToday: reservations.filter((r) => r.check_in_date === today)
            .length,
        checkOutsToday: reservations.filter((r) => r.check_out_date === today)
            .length,
        activeGuests: reservations.filter(
            (r) => r.booking_status === "checked_in",
        ).length,
    ...
    useEffect(() => {
        ...
        const loadReservations = async (showLoading = false) => {
            ...
            try {
                ...
                if (!mounted) return;
                const nextReservations = response.data.data;
                setReservations((previous) => {
                    const previousIds = new Set(
                        previous.map((reservation) => reservation.id),
                    );
                    const latest = nextReservations.find(
                        (reservation) => !previousIds.has(reservation.id),
                    );
    ...
    const handleUpdateStatus = async (id, newStatus) => {
        try {
            const response = await window.axios.patch(
                route("seller.bookings.status", { booking: id }),
                { status: newStatus },
            );
    ...
    const filteredReservations = reservations.filter((r) => {
        if (statusFilter !== "all" && r.booking_status !== statusFilter)
            return false;
        if (searchQuery) {
            ...
            <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
                ...
                <div className="md:flex md:items-center md:justify-between mb-6">
                    <div className="flex-1 min-w-0">
                        <h2 className="text-2xl font-bold leading-7 text-[#1B4D3E] sm:text-3xl sm:truncate">
                            Reservations
                        </h2>
                        <p className="mt-1 text-sm text-slate-500">
                            Manage incoming bookings, check-ins, and guest
                            communication.
                        </p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                    {[
                        ...
                        >
                            <div
                                className={`p-3 rounded-full bg-slate-50 ${stat.color} mr-4`}
                            >
                                <stat.icon
                                    className={`h-6 w-6 ${stat.flip ? "rotate-180" : ""}`}
                                />
                            </div>
                            <div>
                    ...
                    {isLoading ? (
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm font-medium text-slate-900 flex items-center">
                                                    ...
                                                    <span className="ml-2 text-[10px] bg-slate-100 text-slate-600 px-1 py-0.5 rounded uppercase">
                                                        {
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm text-[#D97706] font-medium">
                                                    Rs.{" "}
                                                    {reservation.total_price_lkr.toLocaleString()}
                                                </div>
                                                <div
                                                    className={`text-xs ${reservation.payment_status === "paid" ? "text-emerald-500" : "text-amber-500"}`}
                                                >
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <button
                                                    ...
                                                    className="text-[#1B4D3E] hover:text-[#113127]"
                                                >
                                                    Manage
                                                </button>
                                            </td>
                        ...
                        <div className="p-6">
                            <h3 className="text-lg font-medium text-slate-900 mb-4">
                                Timeline View (By Check-in Date)
                            </h3>
                            <div className="space-y-8">
                                {/* Group by check-in date for timeline simulation */}
                                {Object.entries(
                                    filteredReservations.reduce((acc, r) => {
                                        (acc[r.check_in_date] =
                                    ...
                                    .map(([date, resArray]) => (
                                        <div
                                            key={date}
                                            className="relative pl-8 border-l-2 border-[#1B4D3E]"
                                        >
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                                                {resArray.map((r) => (
                                                    <div
                                                        ...
                                                        className="bg-[#F8FAFC] border border-slate-200 rounded-lg p-4 cursor-pointer hover:shadow-md transition"
                                                    >
                                                        ...
                                                        <h5 className="font-bold text-[#1B4D3E]">
                                                            {r.guest_name}
                                                        </h5>
                                                        ...
                                                        <div className="flex justify-between items-end">
                                                            ...
                                                            <p className="text-sm font-bold text-[#D97706]">
                                                                Rs.{" "}
                                                                {r.total_price_lkr.toLocaleString()}
                                                            </p>
                                                        </div>
                                                    </div>
                                                ))}
