...
export default function Earnings() {
    ...
    const kpis = useMemo(() => {
        ...
        const totalPlatformFees = transactions.reduce(
            (sum, t) => sum + t.platform_commission,
            0,
        );
        ...
        const profitMargin =
            totalGross > 0 ? ((netProfit / totalGross) * 100).toFixed(1) : 0;
    ...
    const chartData = useMemo(() => {
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            data.push({
                ...
                Expenses:
                    currency === "USD" ? dayExpenses / USD_TO_LKR : dayExpenses,
            });
        }
        return data;
    ...
    const handlePayoutRequest = async () => {
        if (kpis.availableBalance <= 0)
            return alert("No available balance to withdraw.");
            ...
            <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
                ...
                <div className="md:flex md:items-center md:justify-between mb-8">
                    <div className="flex-1 min-w-0">
                        <h2 className="text-2xl font-bold leading-7 text-[#1B4D3E] sm:text-3xl">
                            Financials & Payouts
                        </h2>
                        <p className="mt-1 text-sm text-slate-500">
                            Track revenues, log expenses, and manage your
                            payouts.
                        </p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
                        ...
                        <p className="mt-1 text-xs flex items-center text-emerald-600 font-medium">
                            <ArrowUpRight className="h-3 w-3 mr-1" /> 12% vs
                            last month
                        </p>
                    </div>
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
                        ...
                        <p className="mt-2 text-2xl font-bold text-slate-900">
                            {formatCurrency(kpis.totalPlatformFees)}
                        </p>
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5 border-l-4 border-l-rose-500">
                        ...
                        <p className="mt-2 text-2xl font-bold text-slate-900">
                            {formatCurrency(kpis.totalOperatingExpenses)}
                        </p>
                        <p className="mt-1 text-xs text-slate-500">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                            <h3 className="text-sm font-bold text-slate-900 mb-4 flex items-center justify-between">
                                Payout Account
                                <button className="text-xs text-[#1B4D3E] font-medium hover:underline">
                                    Edit
                                </button>
                            </h3>
                                ...
                                {payouts.slice(0, 2).map((p) => (
                                    ...
                                    >
                                        <div>
                                            ...
                                            <p
                                                className={`text-[10px] font-semibold uppercase ${p.status === "completed" ? "text-emerald-600" : "text-amber-500"}`}
                                            >
                                                {p.status}
                                            </p>
                                        </div>
