...
export default function ReservationEngine() {
    ...
    const [reservations, setReservations] = useState([
        {
            ...
            checkIn: "Oct 24",
            checkOut: "Oct 27",
            room: "Deluxe Eco Cabin",
            status: "Checked-In",
            amount: "LKR 45,000",
            payment: "Paid",
            requests: "Vegan breakfast, Airport pickup",
        },
        {
            id: "RES-002",
            guest: "Sarah Jenkins",
            ...
            checkIn: "Oct 26",
            checkOut: "Oct 29",
            room: "Family Treehouse",
            status: "Approved",
            amount: "LKR 66,000",
            payment: "Paid",
            requests: "Extra cot for baby",
        },
        {
            id: "RES-003",
            guest: "Kasun Perera",
            email: "kasun.p@example.lk",
        ...
        },
        {
            id: "RES-005",
            guest: "David Chen",
            email: "david.c@example.sg",
            phone: "+65 9123 4567",
            checkIn: "Nov 15",
            checkOut: "Nov 18",
            room: "Secret Guided Trek",
            status: "Cancelled",
            amount: "LKR 15,000",
            payment: "Refunded",
            requests: "Allergic to peanuts",
        },
    ]);
    ...
    const handleMessage = () => {
        const phone = selectedGuest.phone.replace(/[^0-9]/g, "");
        const message = encodeURIComponent(
            `Hi ${selectedGuest.guest}, this is your host regarding your stay at ${selectedGuest.room}.`,
        );
        window.open(
    ...
    const handlePrint = (documentType) => {
        ...
        document.title = `${documentType} - ${selectedGuest.id}`;
        window.print();
    ...
    const handleStatusChange = (nextStatus) => {
        setReservations((currentReservations) =>
            currentReservations.map((reservation) =>
                reservation.id === selectedGuest.id
    ...
    return (
        <div className="space-y-6 animate-fadeIn">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
                <div>
                    <h2 className="text-xl font-bold text-gray-900 font-sansDisplay">
                        Reservation Desk
                    </h2>
                    <p className="text-sm text-gray-500">
                        Manage bookings, communicate with guests, and process
                        check-ins.
                    </p>
                </div>
            </div>
            ...
            <div className="flex flex-col lg:flex-row gap-6">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {filteredReservations.map((res) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-4">
                                            <span
                                                className={`px-2.5 py-1 rounded-md text-xs font-bold border ${getStatusColor(res.status)}`}
                                            >
                                                {res.status}
                                            </span>
                                        </td>
                                ...
                                {filteredReservations.length === 0 && (
                                    <tr>
                                        ...
                                        >
                                            No reservations found matching this
                                            filter.
                                        </td>
                                    </tr>
                                )}
                ...
                {selectedGuest ? (
                    <div className="w-full lg:w-96 bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col animate-slideInRight">
                        <div className="flex items-start justify-between mb-6">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="font-bold text-gray-900 font-sansDisplay text-lg leading-none">
                                        {selectedGuest.guest}
                                    </h3>
                            ...
                            <span
                                className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase border ${getStatusColor(selectedGuest.status)}`}
                            >
                        ...
                        <div className="mt-6 pt-6 border-t border-gray-100 grid grid-cols-2 gap-3">
                            <button
                                ...
                                className="flex items-center justify-center gap-2 py-2 px-3 bg-white border border-gray-200 text-gray-700 rounded-xl text-xs font-semibold hover:bg-gray-50 transition-colors"
                            >
                            ...
                            {selectedGuest.status === "Approved" && (
                                <button
                                    onClick={() =>
                                        handleStatusChange("Checked-In")
                                    }
                                    className="flex items-center justify-center gap-2 py-2 px-3 bg-[#1B4D3E] text-white rounded-xl text-xs font-semibold hover:bg-[#143d31] transition-colors"
                                >
                            ...
                            {selectedGuest.status === "Checked-In" && (
                                <button
                                    onClick={() =>
                                        handleStatusChange("Checked-Out")
                                    }
                                    className="flex items-center justify-center gap-2 py-2 px-3 bg-gray-800 text-white rounded-xl text-xs font-semibold hover:bg-gray-900 transition-colors"
                                >
                    ...
                    <div className="hidden lg:flex w-96 bg-gray-50/50 rounded-2xl border border-dashed border-gray-200 flex-col items-center justify-center text-center p-8">
                        ...
                        <p className="text-sm text-gray-500">
                            Select a reservation from the table to view guest
                            details and actions.
                        </p>
                    </div>
                )}
