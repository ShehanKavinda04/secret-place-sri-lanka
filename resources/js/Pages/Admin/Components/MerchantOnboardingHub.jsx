    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 animate-fadeIn">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6">
                <div>
                    <h2 className="text-lg font-bold text-slate-900">{t('Merchant Onboarding Hub')}</h2>
                    <p className="text-sm text-gray-500">{t('Review and verify incoming host and merchant applications.')}</p>
                </div>
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-slate-50">
                        <tr>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('Application ID')}</th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('Business Name')}</th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('Type')}</th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('Docs')}</th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('Risk Score')}</th>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {app.docs === 'verified' ? (
                                        <span className="flex items-center text-emerald-600 font-medium">
                                            <ShieldCheck className="w-4 h-4 mr-1" /> {t('Verified')}
                                        </span>
                                    ) : (
                ...
                {filtered.length === 0 && (
                    <div className="text-center py-12 text-gray-500">
                        {t('No pending applications found for this filter.')}
                    </div>
                )}
            </div>
                    ...
                    <div className="p-6">
                        <div className="flex justify-between items-start mb-6 border-b border-gray-100 pb-4">
                            <div>
                                ...
                                <p className="text-sm text-gray-500">{t('Application ID:')} {selectedApplication.id} • {t('Submitted on')} {selectedApplication.date}</p>
                            </div>
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                                {selectedApplication.type} {t('Application')}
                            </span>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-6 mb-6">
                            <div className="bg-slate-50 p-4 rounded-lg border border-gray-200">
                                <h4 className="text-sm font-semibold text-slate-900 mb-3 flex items-center">
                                    <FileText className="w-4 h-4 mr-2 text-indigo-500" /> {t('Document Verification')}
                                </h4>
                                <ul className="space-y-2 text-sm">
                                    <li className="flex justify-between"><span className="text-gray-600">{t('Business Reg (BR):')}</span> <span className="text-emerald-600 font-medium flex items-center"><ShieldCheck className="w-3 h-3 mr-1"/> {t('Valid')}</span></li>
                                    <li className="flex justify-between"><span className="text-gray-600">{t('Owner NIC/Passport:')}</span> <span className="text-emerald-600 font-medium flex items-center"><ShieldCheck className="w-3 h-3 mr-1"/> {t('Valid')}</span></li>
                                    <li className="flex justify-between"><span className="text-gray-600">{t('Bank Details:')}</span> <span className="text-amber-500 font-medium">{t('Awaiting check')}</span></li>
                                </ul>
                            </div>
                            <div className="bg-slate-50 p-4 rounded-lg border border-gray-200">
                                <h4 className="text-sm font-semibold text-slate-900 mb-3 flex items-center">
                                    <AlertCircle className="w-4 h-4 mr-2 text-indigo-500" /> {t('Automated Risk Analysis')}
                                </h4>
                                <p className="text-sm text-gray-600 mb-2">{t('The system has flagged this application as')} <strong className={`capitalize ${selectedApplication.risk === 'high' ? 'text-red-600' : 'text-emerald-600'}`}>{selectedApplication.risk} {t('Risk')}</strong>.</p>
                                {selectedApplication.risk === 'high' && (
                                    <p className="text-xs text-red-600 bg-red-50 p-2 rounded">{t('Warning: Bank account name does not match Business Registration precisely.')}</p>
                                )}
                            </div>
                        </div>
