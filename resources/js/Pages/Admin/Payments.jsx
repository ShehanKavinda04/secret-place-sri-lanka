...
import { 
    ...
    ChevronRight, Receipt, X, FileText, CheckCircle2, AlertCircle, Code
} from 'lucide-react';
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <div className="overflow-x-auto">
                        {activeTab === 'logs' ? (
                            <div className="p-6 bg-gray-50 border-y border-gray-200 font-mono text-xs">
                                {payments.data.length > 0 ? (
                                    <div className="space-y-4">
                                        {payments.data.map((payment) => (
                                            <div key={payment.id} className="bg-slate-900 text-green-400 p-4 rounded-md overflow-x-auto shadow-sm">
                                                <div className="text-gray-400 mb-2 border-b border-gray-700 pb-2 flex justify-between">
                                                    <span>[{new Date(payment.updated_at).toISOString()}] GATEWAY_WEBHOOK: {payment.gateway ? payment.gateway.toUpperCase() : 'SYSTEM'}</span>
                                                    <span className={payment.status === 'success' ? 'text-green-500' : (payment.status === 'refunded' ? 'text-yellow-500' : 'text-red-500')}>HTTP 200 OK</span>
                                                </div>
                                                <pre>
{JSON.stringify({
    event: `charge.${payment.status}`,
    id: payment.transaction_id || `txn_sys_${payment.id}`,
    ...
    metadata: {
        booking_ref: `BKG-${payment.booking_id}`,
        customer_email: payment.booking?.tourist?.email || 'N/A',
        merchant_account: payment.booking?.business?.owner?.email || 'N/A'
    }
}, null, 2)}
                                                </pre>
                                            </div>
                                    ...
                                    <div className="text-center text-gray-500 py-12 flex flex-col items-center">
                                        ...
                                        <p className="text-sm mb-4">Try adjusting your filters to see gateway events.</p>
                                    </div>
                                )}
                            </div>
                        ) : (
