...
match = re.search(
    r"'isurumuniya'.*?'history_narrative' => <<<'EOD'\n(.*?)\nEOD",
    route_source,
    re.DOTALL,
)
if not match:
    raise RuntimeError('Isurumuniya narrative not found')

text = re.sub(r'\$([^$]+)\$', r'\1', match.group(1)).replace('\\', '')
output_path = os.path.join(os.path.dirname(__file__), 'public', 'audio', 'isurumuniya_history.mp3')
