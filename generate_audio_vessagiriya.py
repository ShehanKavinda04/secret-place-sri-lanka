...
match = re.search(
    r"'vessagiriya'.*?'history_narrative' => <<<'EOD'\n(.*?)\nEOD",
    route_source,
    re.DOTALL,
)
if not match:
    raise RuntimeError('Vessagiriya narrative not found')

text = re.sub(r'\$([^$]+)\$', r'\1', match.group(1)).replace('\\', '')
output_path = os.path.join(os.path.dirname(__file__), 'public', 'audio', 'vessagiriya_history.mp3')
